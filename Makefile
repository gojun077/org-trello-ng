...
# Create or reuse a GitHub PR. Usage: make pr BEADS_ID=<id>
pr:
ifndef BEADS_ID
	...
	if [ -n "$$PR_URL" ]; then \
		...
		PR_BODY=$$(bd show "$(BEADS_ID)" --json | jq -r '.[0].description'); \
		gh pr create \
			--base main \
			--draft \
			$$LABEL_ARGS \
			--title "Fix($(BEADS_ID)): $$(git log -1 --pretty=%s)" \
			--body "## Beads Task: $(BEADS_ID)$$(printf '\n\n### Description\n')$$PR_BODY$$(printf '\n\n---\n*Generated by Agent in Worktree: ')$$(hostname)*"; \
	fi

# Full session completion. Usage: make finish BEADS_ID=<id>
finish:
ifndef BEADS_ID
...
help:
	...
	@echo "  pr     - Create/reuse GitHub PR (BEADS_ID=<id>)"
	@echo "  finish - Full session completion (BEADS_ID=<id>)"
	@echo ""
	@echo "Variables:"
